/*
  ** @file           : drv_globalmen_multidev.c
  ** @brief          : global memory driver multi device source file
  **
  ** @attention
  **
...
static int __init globalmem_init(void)
{
    ...
    if (globalmem_major) 
      ...
      ret = register_chrdev_region(devno, GLOBALMEN_DEV_SIZE, "globalmem");
    else {
      // ret = alloc_chrdev_region(&devno, 0, 1, "globalmem");
      ret = alloc_chrdev_region(&devno, 0, GLOBALMEN_DEV_SIZE, "globalmem");      
    ...
    globalmem_devp = kzalloc(sizeof(struct globalmem_dev) * GLOBALMEN_DEV_SIZE, GFP_KERNEL);    
    if (!globalmem_devp) {
      ret = -ENOMEM;
      goto fail_malloc;
    }
    ...
    for(cnt = 0; cnt < GLOBALMEN_DEV_SIZE; cnt++)
      globalmem_setup_cdev(globalmem_devp + cnt, cnt);

    return 0; 

fail_malloc:
    // unregister_chrdev_region(devno, 1);
    unregister_chrdev_region(devno, GLOBALMEN_DEV_SIZE);    
    return ret;
}
...
static void __exit globalmem_exit(void)
{
    ...
    int cnt;
    for (cnt = 0; cnt < GLOBALMEN_DEV_SIZE; cnt++) 
      cdev_del(&(globalmem_devp + cnt)->cdev);

    kfree(globalmem_devp);
    // unregister_chrdev_region(MKDEV(globalmem_major, 0), 1);
    unregister_chrdev_region(MKDEV(globalmem_major, 0), GLOBALMEN_DEV_SIZE);    
}
